  ...
  const formatMessage = (content: string) => {
    ...
    return content
      ...
      .map((line, index) => {
        ...
        if (line.startsWith('✨')) {
          return <div key={index} className="flex items-center gap-2 text-green-600"><Sparkles className="w-4 h-4" />{line.slice(1)}</div>;
        }
        if (line.startsWith('🧪')) {
          return <div key={index} className="flex items-center gap-2 text-blue-600"><TestTube className="w-4 h-4" />{line.slice(1)}</div>;
        }
        if (line.startsWith('⚡')) {
          return <div key={index} className="flex items-center gap-2 text-yellow-600"><Zap className="w-4 h-4" />{line.slice(1)}</div>;
        }
        if (line.startsWith('📋')) {
          return <div key={index} className="flex items-center gap-2 text-purple-600"><Github className="w-4 h-4" />{line.slice(1)}</div>;
        }
        if (line.startsWith('🚀')) {
          return <div key={index} className="flex items-center gap-2 text-orange-600"><ExternalLink className="w-4 h-4" />{line.slice(1)}</div>;
        }
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-xl text-gray-600 max-w-3xl mx-auto leading-relaxed">
              ...
              execution, and GitHub issue creation
            </p>
            ...
            <CardContent className="p-6">
              ...
              <div className="mt-6 flex flex-wrap gap-3">
                ...
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => setInput("Generate tests for https://example.com")}
                  className="text-sm bg-white/50 backdrop-blur-sm hover:bg-white/80"
                >
